...
public class EmfMavenFileCreator {
	...
	private static final String[] plugins = { "org.eclipse.emf.codegen.ecore",
			"org.eclipse.emf.codegen", "org.eclipse.emf.common",
			"org.eclipse.emf.ecore", "org.eclipse.emf.ecore.xmi",
			"org.eclipse.xsd", "org.eclipse.emf.mapping.ecore2xml",
			"org.eclipse.emf.edit", "org.eclipse.emf.databinding",
	...
	private void process() throws Exception {
		...
		for (String plugin : plugins) {
			for (File file : mainDir.listFiles()) {
				...
				copyFile(sourceDirPath + fileName, outDirPath + plugin + "-"
						+ version + ".jar");

				// copy the source
				final String sourceOriginFileName = fileName.replace("_",
						".source_");
				copyFile(sourceDirPath + sourceOriginFileName, outDirPath
						+ plugin + "-" + version + "-sources.jar");

				// copy the javadoc template
				copyResource(JAVADOC_TEMPLATE_JAR_FILE, outDirPath + plugin
						+ "-" + version + "-javadoc.jar");

				// copy the pom

				File pomTemplate = (new File(this.getClass()
						.getResource(POM_TEMPLATE_FILE).toURI()));
				String pom = new String(FileUtils.readFileToString(pomTemplate,
				...
				FileUtils.writeByteArrayToFile(new File(outDirPath + plugin
						+ "-" + version + ".pom"), pom.getBytes());

				if (isSnapShot()) {
					System.err.println("<deploy.snapshot name=\"" + plugin
							+ "\" version=\"" + version
							+ "\" jarFileLocation=\"${mainLocation}/" + dirName
							+ "\"/>");
				} else {
					System.err.println("<deploy.staging name=\"" + plugin
							+ "\" version=\"" + version
							+ "\" jarFileLocation=\"${mainLocation}/" + dirName
							+ "\"/>");
				}
	...
	private void copyResource(String sourceFilePath, String destFilePath)
			throws Exception {
		File srcFile = new File(this.getClass().getResource(sourceFilePath)
				.toURI());
		File destFile = new File(this.getClass().getResource(destFilePath)
				.toURI());
